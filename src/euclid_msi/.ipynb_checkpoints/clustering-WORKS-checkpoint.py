...
class Clustering:
    ...
    def __init__(self, emb,
                max_depth = 0, ############ # go down where possible! it should automatically stop before everywhere given the biological plausibility criteria!
        ...
        self.min_diff_lipids = min_diff_lipids
        self.min_fc = min_fc
        self.pthr = pthr
        ...
        try:
            ...
            self.standardized_embeddings_GLOBAL = pd.DataFrame(StandardScaler().fit_transform(emb.adata.obsm['X_NMF']),
                                                          index=emb.adata.obs_names)
        coordinates = emb.adata.obs[['x','y','SectionID', 'SectionID']] ###################
        ...
        column_names = [f"level_{i}" for i in range(1,self.max_depth+1)]
        self.clusteringLOG = pd.DataFrame(0, index=data.index, columns=column_names)[::DSFACT]  
        ...
        self.vcnorm = self.coordinates.loc[data.index,'Section'].value_counts()
        self.vcnorm.index = self.vcnorm.index.astype(int)
        self.vcnorm = self.vcnorm.sort_index()
        
        sections = np.sort(self.coordinates['Section'].unique())
        n_sections = len(sections)

        threshold = 15

        if n_sections > threshold:
            # --------------------------
            # (> threshold) use original logic
            # --------------------------
            valsec = (sections[::5] + 2)[:-1]
            testsec = (sections[::5] + 1)[:-1]
            # trainsec is everything else
            trainsec = np.setdiff1d(np.setdiff1d(sections, testsec), valsec)

            self.valpoints = self.coordinates.loc[
                self.coordinates['Section'].isin(valsec), :
            ].index
            self.testpoints = self.coordinates.loc[
                self.coordinates['Section'].isin(testsec), :
            ].index
            self.trainpoints = self.coordinates.loc[
                self.coordinates['Section'].isin(trainsec), :
            ].index

        else:
            # --------------------------
            # (≤ threshold) fallback: random 70/20/10 split of all points
            # --------------------------
            all_points = self.coordinates.index.to_numpy()
            # For reproducibility, you can fix a random seed:
            rng = np.random.RandomState(42)
            permuted = rng.permutation(all_points)

            n_total = len(permuted)
            n_train = int(n_total * 0.70)
            n_val   = int(n_total * 0.20)
            # Remaining become test (≈10%)
            n_test  = n_total - n_train - n_val

            self.trainpoints = permuted[:n_train]
            self.valpoints   = permuted[n_train : n_train + n_val]
            self.testpoints  = permuted[n_train + n_val : ]

    def compute_seeded_NMF(self, data):  # data is a dataframe pixels x lipids
        ...
        adatatmp = anndata.AnnData(X=np.zeros_like(corr_matrix))
        adatatmp.obsp['connectivities'] = csr_matrix(corr_matrix)
        adatatmp.uns['neighbors'] = {
        ...
        for gamma in gamma_values:
            sc.tl.leiden(adatatmp, resolution=gamma, key_added=f'leiden_{gamma}')
            clusters = adatatmp.obs[f'leiden_{gamma}'].astype(int).values
        ...
        sc.tl.leiden(adatatmp, resolution=best_gamma, key_added='leiden_best') # run Leiden one final time with best parameters
        clusters = adatatmp.obs['leiden_best'].astype(int).values
        #print(clusters)
    ...
    def plot_spatial_localPCA(self, spat, pc_top):
    ...
    def plot_spatial_localPCA_kMeans(self, dd2, kmeans_labels):
    ...
    def plot_embeddingsPCA(self, embeddings, kmeans_labels):
    ...
    def plot_tSNE(self, indexes, kmeans_labels):
    ...
    def continuity_check(self, dd2, kmeans_labels):
        dd2 = pd.DataFrame(dd2, columns = ['zccf','yccf','Section'])
    ...
    def differential_lipids(self, lipidata, kmeans_labels, min_fc=0.2, pthr=0.05):
        results = []
        ...
        promoted = results_df.loc[(results_df['log2fold_change'] > self.min_fc) & (results_df['p_value_corrected'] < self.pthr),:]

        alteredlips = np.sum((results_df['log2fold_change'] > self.min_fc) & (results_df['p_value_corrected'] < self.pthr))
    ...
    def rank_features_by_combined_score(self, tempadata):
    ...
    def generate_combinations(self, n, limit=3): # 200 ######################
        all_combinations = []
    ...
    def leidenalg_clustering(self, inputdata, Nneigh=40, Niter=5):
    ...
    def undersample(self, X, y, sampling_strategy='auto'):
        rus = RandomUnderSampler(sampling_strategy=sampling_strategy, random_state=42)
        X_resampled, y_resampled = rus.fit_resample(X, y)
        return X_resampled, y_resampled

    def dosplit(self, current_adata, embds, path = [], splitlevel=0, plotting_active=False, loadings_active=False):
        print("Splitting level: "+str(splitlevel))

        # do not split further if the cluster is smaller than self.min_voxels
        if current_adata.X.shape[0] < self.min_voxels:
            print("one branch exhausted")
            return

        # do NMF on the current subset of the data
        nmfdf, loadings, N_factors, nmf = self.compute_seeded_NMF(pd.DataFrame(current_adata.X))
        nmf_result = nmfdf.values

        # omit NMFs with very low signal overall
        filter1 = np.abs(nmf_result).mean(axis=0) > self.thr_signal
        ...
        goodpcs = self.rank_features_by_combined_score(tempadata)
        ...
        flag = False
        aaa = 0
        multiplets = self.generate_combinations(len(goodpcs))
        ...
        while (not flag) and (aaa < len(multiplets)):
            ...
            kmeans = KMeans(n_clusters=self.K, random_state=230598)
            globembds = self.standardized_embeddings_GLOBAL.loc[current_adata.obs_names,:].values
            embspace = np.concatenate((standardized_embeddings, embds/self.penalty1, globembds/self.penalty2),axis=1)
            ...
            enough_sections0, enough_sections1, number_of_peaks0, number_of_peaks1, peak_ratio0, peak_ratio1 = self.continuity_check(current_adata.obsm['spatial'], np.array(data_for_clustering['lab']))
            
            print(enough_sections0)
            print(enough_sections1)
            print(number_of_peaks0)
            print(number_of_peaks1)
            print(peak_ratio0)
            print(peak_ratio1)

            # check the differential lipids
            alteredlips, promoted = self.differential_lipids(current_adata.obsm['lipids'].values, kmeans_labels, self.min_fc, self.pthr)

            print(promoted)
            print(np.sum(kmeans_labels == 1))
            ...
            flag = ((np.sum(kmeans_labels == 1) > self.min_voxels) | (np.sum(kmeans_labels == 0) > self.min_voxels)) & (alteredlips > self.min_diff_lipids) & enough_sections0 and  enough_sections1 and ((number_of_peaks0 <3) or (peak_ratio0 > 1.4)) and ((number_of_peaks1 <3) or (peak_ratio1 > 1.4))
            aaa = aaa+1
        ...
        X_train = embeddings.loc[embeddings.index.isin(self.trainpoints), :]
        X_val = embeddings.loc[embeddings.index.isin(self.valpoints), :]
        X_test = embeddings.loc[embeddings.index.isin(self.testpoints), :]
        ...
        X_train_sub_US, y_train_sub_US = self.undersample(X_train, y_train)
        ...
        node = self.Node(splitlevel, path=path)
        node.scaler = scaler
        node.nmf = nmf
        ...
        self.clusteringLOG.loc[cl1members, "level_"+str(splitlev)] = 1
        self.clusteringLOG.loc[cl2members, "level_"+str(splitlev)] = 2
        ...
        """
        # do the recursion
        if splitlev < (self.max_depth+1):
            child_path0 = path + [0]
            child_path1 = path + [1]
            childnode0, childnode1 = self.dosplit(current_adata1, embd1, child_path0, splitlevel=splitlev), self.dosplit(current_adata2, embd2, child_path1, splitlevel=splitlev)
    ...
    def learn_euclid_clustering(self):
        DSFACT = 1 # downscale for testing if needed
        column_names = [f"level_{i}" for i in range(1,self.max_depth+1)]
        tree = pd.DataFrame(0, index=self.coordinates.index, columns=column_names)[::DSFACT]   
        root_node = self.dosplit(self.adatamaia[::DSFACT], self.standardized_embeddings_GLOBAL[::DSFACT], splitlevel=0)
        ...
        tree = self.clusteringLOG
        #tree['class'] = tree['level_1'].astype(str) + tree['level_2'].astype(str) + tree['level_3'].astype(str)
        tree.to_hdf("tree_clustering_euclid.h5ad", key="table")
        
        ### SAVE THE ROOT NODE... AND MAKE A SELF FOR IT...
        return root_node, tree
        
    def traverse_tree(self, node, current_adata, embds, level=0):
        print(level)
        ...
        globembds = self.standardized_embeddings_GLOBAL.loc[current_adata.obs_names].values / self.penalty2
        embspace = np.concatenate((X_scaled, embds / self.penalty1, globembds), axis=1)
        
        print(embspace.shape)
        ...
        for i, index in enumerate(current_adata.obs_names):
            if index not in self.paths:
                self.paths[index] = []
            self.paths[index].append(child_labels[i])
        ...
        self.traverse_tree(node.children[0], current_adata0, embd0, level + 1)
        self.traverse_tree(node.children[1], current_adata1, embd1, level + 1)
    
    def apply_euclid_clustering(self, root_node, adatatorun):
        DSFACT = 1
        embds = self.standardized_embeddings_GLOBAL[::DSFACT].values
        self.traverse_tree(root_node, adatatorun[::DSFACT], embds)
        df_paths = pd.DataFrame.from_dict(self.paths, orient='index')
        ...
        """
        df_paths.to_hdf("df_transferred_euclid.h5ad", key="table")
        print(self.paths)
        print("BELLA")
        return df_paths
